...
func validateAPIKEY(c *gin.Context) error {
	...
	switch serviceName {
	case "field-services":
		signatureKey = config.Cfg.InternalService.Field.SignatureKey
	case "user-services":
		signatureKey = config.Cfg.InternalService.User.SignatureKey
	case "payment-services":
		signatureKey = config.Cfg.InternalService.Payment.SignatureKey
	case "order-services":
		signatureKey = config.Cfg.SignatureKey
	default:
...
func CheckRole(roles []string, client clients.IClientRegistry) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		userLogin := ctx.Request.WithContext(context.WithValue(ctx.Request.Context(), constants.User, user))
